...
if (json) process.stdout.write(`${JSON.stringify(report, null, 2)}\n`);
  ...
  for (const result of results) {
    process.stdout.write(`${result.caseId} ${result.variant} #${result.repetition}: ${result.decision} (${result.by}; intent=${result.intentMismatch ?? "n/a"}; scope=${result.wrongScope ?? "n/a"})\n`);
  }
}
